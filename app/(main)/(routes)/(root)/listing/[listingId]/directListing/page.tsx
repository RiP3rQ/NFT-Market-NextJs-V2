...
const ListingPage = ({ params }: { params: { listingId: string } }) => {
  ...
  const listingTypeURL = searchParams?.get("listingType");
  ...
  const { data: DirectListing, isLoading } = useDirectListing(
    contract,
    listingId
  );
  const {
  ...
  if (!listing) {
    return (
      <div>
        Listing not found! ${listingId} ${listingTypeURL}
      </div>
    );
  }
